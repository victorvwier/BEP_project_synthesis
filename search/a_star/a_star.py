...
class AStar(SearchAlgorithm):
    ...
    def setup(self, training_examples: List[Example], trans_tokens: set[Token], bool_tokens: set[Token]):
        ...
        print(f"input envs: {len(self.input_envs)}")
    ...
    def extend_result(self, search_result: SearchResult):
        ...
        if self._solution_found:
            ...
            if self._solution_found:
                ...
                print(f"no. tokens: {self._best_program.number_of_tokens(control_cost=1)}")
                print(f"iterations: {self._iteration_count}")
            search_result.dictionary['graph'] = nx.node_link_data(G)

        search_result.dictionary['best_program_length'] = self._best_program.number_of_tokens(control_cost=1)
    ...
    def best_first_search_upq(self, start_node: tuple[Environment], end_node: tuple[Environment],
        ...
        while queue:
            ...
            # print(horizontal_string_join(node[0].to_formatted_string() + "\n", end_node[0].to_formatted_string()))
            ...
            # print(f"gcost: {gcost}")
            # print(f"hcost: {hcost}")
            # print(self._expanded_programs[-1].to_formatted_string() + "\n")
            ...
            for token, node_copy in zip(tokens, node_copies):
                try:
                    ...
                    gcost_child = gcost + token.number_of_tokens(control_cost=1)
                    ...
                    pass
            # if solution:
            #     program = self._find_program(solution, reached)
            #     self._best_program_hcost = h(solution, end_node)
            #     yield program
            # else:
            #     yield None
        return
...
class Dijkstra(AStar):
    def __init__(self, time_limit_sec: float):
        super().__init__(time_limit_sec)
        self.loss_function = lambda g, h: g
        self.heuristic = self._heuristic_sum